...
const $ = {
  post: (params, callback) => {
    ...
    // 确保 timeout 是对象格式
    if (typeof others.timeout === 'number') {
      others.timeout = { request: others.timeout };
    }
    // 如果没有 timeout，使用全局的
    if (others.timeout === undefined) {
      others.timeout = timeout;
    }
